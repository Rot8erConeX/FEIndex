...
@names=[["Aqua","Azura"],["Joker","Jakob"],["Gunther","Gunter"],["Kazahana","Hana"],["Tsubaki","Subaki"],["Suzukaze","Kaze"],["Tsukuyomi","Hayato"],
        ...
        ["Velour", "Velouria"],["Reflet","Robin"],["Olivie","Olivia"]]
...
def generation_parse(event,bob1=nil,bob2=nil,bob3=nil,bob4=nil,permerror=true)
  ...
  if !homosexuality_filter?(event)
    if !grandmother.nil? && ["Awakening","*Awakening*"].include?(f2[1])
    ...
    elsif !mother.nil? && ["Awakening","*Awakening*"].include?(f[1])
    ...
    elsif !grandmother.nil? && !["Awakening","*Awakening*"].include?(f2[1])
...
def class_parse(event,bot,args)
  ...
  apt=20 if clss[19]=="Awakening" && apt>0
  ...
  if event.message.text[0,3].downcase=="fe!" && find_class(args.join(' ').downcase,event,"Fates")!=find_class(args.join(' ').downcase,event,"Awakening")
    if @embedless.include?(event.user.id) || was_embedless_mentioned?(event)
      ...
      apt=20 if apt>0
      for i in 0...8
        text="#{text}\n**#{b[i]}:**	*Growth:* #{clss2[1+2*i].to_i+apt}%	*Base:* #{clss2[20+i]}	*Maximum:* #{clss2[2+2*i]}"
      end
      text="#{text}\n#{clss2[17]}"
      text="#{text}\n**Class skills:**	#{clss2[18]}" unless clss2[18].nil? || clss2[18]==""
      text="#{text}\n\n__*Fates*__"
      apt=10 if apt>0
      for i in 0...8
        text="#{text}\n**#{b[i]}:**	*Growth:* #{clss[1+2*i].to_i+apt}%	*Base:* #{clss[20+i]}	*Maximum:* #{clss[2+2*i]}"
      end
      text="#{text}\n#{clss[17]}"
      text="#{text}\n**Class skills:**	#{clss[18]}" unless clss[18].nil? || clss[18]==""
      xcolor=0x010101
      ...
      flds[0][1]="#{flds[0][1]}\n\n*Total*: #{clss[1].to_i+clss[3].to_i+clss[5].to_i+clss[7].to_i+clss[9].to_i+clss[11].to_i+clss[13].to_i+clss[15].to_i}%"
      flds[1][1]="#{flds[1][1]}\n\n*Total*: #{clss[20].to_i+clss[21].to_i+clss[22].to_i+clss[23].to_i+clss[24].to_i+clss[25].to_i+clss[26].to_i+clss[27].to_i}"
      flds[2][1]="#{flds[2][1]}\n\n*Total*: #{clss[2].to_i+clss[4].to_i+clss[6].to_i+clss[8].to_i+clss[10].to_i+clss[12].to_i+clss[14].to_i+clss[16].to_i}"
      text="#{text}\n**Promotes from:** Discord bot" if clss[0]=="Automaton"
      text="#{text}\n#{clss[17]}"
      text="#{text}\n**Class skills:** #{clss[18]}" unless clss[18].nil? || clss[18]==""
      xcolor=0x061069 if clss[19]=="Awakening"
      if clss[19]=="Fates"
        xcolor=0xC5EEF2
        xcolor=0xCF000D if clss[28]=="Hoshido"
        xcolor=0xAA7FCD if clss[28]=="Nohr"
        xcolor=0x2DA5AF if clss[28]=="Valla"
        xcolor=0xFFEA8B if clss[28]=="Penumbra"
      end
      create_embed(event,fullname,text,xcolor,nil,nil,flds)
      flds=[["**Growths**",""],["**Bases**",""],["**Maximums**",""]]
      fullname="__**#{clss[0].gsub(' (C)','')}**__"
      text=''
      for i in 0...8
        flds[0][1]="#{flds[0][1]}\n*#{b[i]}:*	#{clss2[1+2*i].to_i+apt}%"
        flds[1][1]="#{flds[1][1]}\n*#{b[i]}:*	#{clss2[20+i].to_i}"
        flds[2][1]="#{flds[2][1]}\n*#{b[i]}:*	#{clss2[2+2*i].to_i}"
      end
      flds[0][1]="#{flds[0][1]}\n\n*Total*: #{clss2[1].to_i+clss2[3].to_i+clss2[5].to_i+clss2[7].to_i+clss2[9].to_i+clss2[11].to_i+clss2[13].to_i+clss2[15].to_i}%"
      flds[1][1]="#{flds[1][1]}\n\n*Total*: #{clss2[20].to_i+clss2[21].to_i+clss2[22].to_i+clss2[23].to_i+clss2[24].to_i+clss2[25].to_i+clss2[26].to_i+clss2[27].to_i}"
      flds[2][1]="#{flds[2][1]}\n\n*Total*: #{clss2[2].to_i+clss2[4].to_i+clss2[6].to_i+clss2[8].to_i+clss2[10].to_i+clss2[12].to_i+clss2[14].to_i+clss2[16].to_i}"
      text="#{text}\n**Promotes from:** Discord bot" if clss2[0]=="Automaton"
      text="#{text}\n#{clss2[17]}"
      text="#{text}\n**Class skills:** #{clss2[18]}" unless clss2[18].nil? || clss2[18]==""
      xcolor=0x061069 if clss2[19]=="Awakening"
      if clss2[19]=="Fates"
        xcolor=0xC5EEF2
        xcolor=0xCF000D if clss2[28]=="Hoshido"
        xcolor=0xAA7FCD if clss2[28]=="Nohr"
        xcolor=0x2DA5AF if clss2[28]=="Valla"
        xcolor=0xFFEA8B if clss2[28]=="Penumbra"
      end
      create_embed(event,fullname,text,xcolor,nil,nil,flds)
    end
  else
...
def skill_parse(event,bot,args)
  ...
  if event.message.text[0,3].downcase=="fe!" && find_skill("Fates",name,event)!=find_skill("Awakening",name,event)
    ...
    text=''
    if skillA[1]!=skillF[1] || skillA[2]!=skillF[2]
      if skillA[2]=="-"
        if skillA[1].downcase=="enemy exclusive"
          text="#{text}\n*Awakening*: #{skillA[1]}"
        elsif skillA[1]=="-"
          text="#{text}\n*Awakening*: Possible personal skill for Bond Units"
        elsif skillA[1].length<7
          text="#{text}\n*Awakening*: `#{skillA[1]}`'s personal skill"
        elsif skillA[1][skillA[1].length-7,7].downcase==" manual"
          text="#{text}\n*Awakening*: Learned via #{skillA[1]}"
        else
          text="#{text}\n*Awakening*: `#{skillA[1]}`'s personal skill"
        end
      else
        text="#{text}\n*Awakening*: Learned at level #{skillA[2]} by #{skillA[1]}"
      end
      if skillF[2]=="-"
        if skillF[1].downcase=="enemy exclusive"
          text="#{text}\n*Fates*: #{skillF[1]}"
        elsif skillF[1]=="-"
          text="#{text}\n*Fates*: Possible personal skill for Bond Units"
        elsif skillF[1].length<7
          text="#{text}\n*Fates*: `#{skillF[1]}`'s personal skill"
        elsif skillF[1][skillF[1].length-7,7].downcase==" scroll"
          text="#{text}\n*Fates*: Learned via #{skillF[1]}"
        else
          text="#{text}\n*Fates*: `#{skillF[1]}`'s personal skill"
        end
      else
        text="#{text}\n*Fates*: Learned at level #{skillF[2]} by #{skillF[1]}"
      end
    elsif bob4[2]=="-"
      if bob4[1].downcase=="enemy exclusive"
        text="#{text}\n#{bob4[1]}"
      elsif bob4[1]=="-"
        text="#{text}\nPossible personal skill for Bond Units"
      elsif bob4[1].length<7
        text="#{text}\n`#{bob4[1]}`'s personal skill"
      elsif [" scroll"," manual"].include?(bob4[1][bob4[1].length-7,7].downcase)
        text="#{text}\nLearned via #{bob4[1]}"
      else
        text="#{text}\n`#{bob4[1]}`'s personal skill"
      end
    else
      text="#{text}\nLearned at level #{bob4[2]} by #{bob4[1]}"
    end
    if bob4[3]!="-"
    ...
    if skillA[4]==skillF[4]
      ...
      text="#{text}\n**Effect:** #{skillA[4]} (In *Fates*, does not work if the user would deal 0 damage)"
    elsif bob4[0]=="Galeforce"
      text="#{text}\n**Effect:** #{skillA[4]} (In *Fates*, this does not trigger in Attack or Guard Stance)"
    else
      text="#{text}\n***Awakening*** **Effect:** #{skillA[4]}"
      text="#{text}\n***Fates*** **Effect:** #{skillF[4]}"
    end
    xcolor=0x010101
  else
    fullname="__**#{bob4[0]}**__"
    ...
    text="#{text}\n**Effect:** #{bob4[4]}"
    xcolor=0x010101
    xcolor=0x061069 if bob4[5]=="Awakening"
...
bot.command(:proc) do |event, *args|
  ...
  names.push("Glacies") if event.user.id==167657750971547648
  names2=[]
  names3=[]
  for i in 0...args.length
